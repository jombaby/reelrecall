...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    const apiResponse=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({
      ...
      input:[{role:"system",content:[{type:"input_text",text:"Classify a saved social video using only the supplied evidence. Select exactly one existing category and one of that category's subcategories, or an empty subcategory. Create 3-6 concise lowercase tags. Never invent a new category. If evidence is weak, use Uncategorized and general factual tags."}]},{role:"user",content:[{type:"input_text",text:JSON.stringify({available_categories:categories,whatsapp_title:body.video.title,whatsapp_note:body.video.notes,public_description:description})}]}],
