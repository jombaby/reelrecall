...
export default function Home(){
  ...
  function saveVideo(e:FormEvent){
    ...
    if(editingId){
      setVideos(a=>a.map(v=>v.id===editingId?{...v,...form,title,url,tags,source:url?sourceFor(url):"Other",titleLocked:true,categoryLocked:true,tagsLocked:true,aiStatus:"done"}:v));
      setToast(url?"Item updated":"Manual item updated");
    }else{
      setVideos(a=>[{id:crypto.randomUUID(),...form,title,url,tags,source:url?sourceFor(url):"Other",favorite:false,addedAt:new Date().toISOString(),titleLocked:true,categoryLocked:true,tagsLocked:true,aiStatus:"done",recipe:null},...a]);
      setToast(url?"Item added":"Manual item added");
    }
    setDialogOpen(false);
  }
