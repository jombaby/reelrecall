...
function descriptionRecipeIsRicher(current:Recipe,candidate:Recipe){
  ...
  const noMajorIngredientRegression=currentIngredients===0||candidateIngredients>=Math.max(1,Math.floor(currentIngredients*.8));
  const noMajorStepRegression=currentSteps===0||candidateSteps>=Math.max(1,Math.floor(currentSteps*.8));
...
function applyRecipeCandidate(video:Video,incoming:Recipe,origin:"description"|"video"|"manual",force=false):Video{
  ...
  if(origin==="description"&&!force){
    if(video.recipe.source==="manual")return video;
    if(!descriptionRecipeIsRicher(video.recipe,candidate))return video;
  }
...
const MANUAL_ITEM_URL_PREFIX="https://manual.reelrecall.local/item/";
function isManualItemUrl(raw:string){return String(raw||"").startsWith(MANUAL_ITEM_URL_PREFIX)}
function hasExternalVideoUrl(raw:string){const value=String(raw||"").trim();return Boolean(value)&&!isManualItemUrl(value)}
function manualItemStorageUrl(id:string){return `${MANUAL_ITEM_URL_PREFIX}${encodeURIComponent(id)}`}
function visibleVideoUrl(raw:string){return hasExternalVideoUrl(raw)?raw:""}

function recipeText(video:Video){const r=video.recipe;if(!r)return"";return[r.title,r.servings?`Servings: ${r.servings}`:"",r.prepTime?`Time: ${r.prepTime}`:"","Ingredients",...r.ingredients.map(x=>`• ${x}`),"","Steps",...r.steps.map((x,i)=>`${i+1}. ${x}`),r.notes?`\nNotes\n${r.notes}`:"",hasExternalVideoUrl(video.url)?`\nSource: ${video.url}`:""].filter(Boolean).join("\n")}
...
function embedUrl(video:Video){if(!hasExternalVideoUrl(video.url))return null;if(video.source==="YouTube"){const id=youtubeId(video.url);return id?`https://www.youtube.com/embed/${id}?autoplay=1&rel=0`:null}if(video.source==="Instagram"){try{const u=new URL(video.url),p=u.pathname.split("/").filter(Boolean);return p[1]?`https://www.instagram.com/reel/${p[1]}/embed/`:null}catch{return null}}return null}
...
function VideoCard({video,playing,onPlay,onEdit,onDelete,onFavorite,onStatus,onRecipe,onManualRecipe,recipeLoading,onAnalyzeVideo,videoAnalyzing,analysisReady}:{video:Video;playing:boolean;onPlay:()=>void;onEdit:()=>void;onDelete:()=>void;onFavorite:()=>void;onStatus:()=>void;onRecipe:()=>void;onManualRecipe:()=>void;recipeLoading:boolean;onAnalyzeVideo:()=>void;videoAnalyzing:boolean;analysisReady:boolean}){
  const[thumbnail,setThumbnail]=useState<string|null>(null),[failed,setFailed]=useState(false);
  const hasUrl=hasExternalVideoUrl(video.url);
  useEffect(()=>{
    let active=true;
    setThumbnail(null);
    setFailed(false);
    if(!hasExternalVideoUrl(video.url)){
      setFailed(true);
      return()=>{active=false};
    }
    fetch(`/api/thumbnail?url=${encodeURIComponent(video.url)}`)
      .then(r=>r.ok?r.json():Promise.reject())
      .then((d:{thumbnail?:string})=>{if(active&&d.thumbnail)setThumbnail(d.thumbnail)})
      .catch(()=>{if(active)setFailed(true)});
    return()=>{active=false};
  },[video.url]);
  const player=hasUrl?embedUrl(video):null,unavailable=video.status==="unavailable",watchLabel=video.source==="Facebook"?"Watch in Facebook ↗":"Open original ↗";
  const sourceLabel=hasUrl?video.source:"Manual";
  return <article data-video-id={video.id} className={`recipe-card ${unavailable?"unavailable":""}`}><div className={`source-badge ${hasUrl?video.source.toLowerCase():"manual"}`}>{sourceLabel}</div><button className={`heart ${video.favorite?"active":""}`} onClick={onFavorite}>♥</button>
    {unavailable?<div className="not-available"><span>{video.source==="Facebook"&&hasUrl?"This reel may no longer be available":"Item marked unavailable"}</span><small>{video.source==="Facebook"&&hasUrl?"You marked this item unavailable. Open Facebook to verify its current state.":"You marked this item unavailable."}</small>{video.source==="Facebook"&&hasUrl?<a href={video.url} target="_blank" rel="noreferrer">Watch in Facebook ↗</a>:null}</div>:hasUrl&&playing&&video.source==="Facebook"?<FacebookEmbed url={video.url} title={video.title} thumbnail={thumbnail&&!failed?thumbnail:null} onThumbnailError={()=>setFailed(true)}/>:hasUrl&&playing&&video.source==="Instagram"?<InstagramEmbed url={video.url} title={video.title}/>:hasUrl&&playing&&player?<div className="inline-player"><iframe src={player} title={video.title} allow="autoplay; encrypted-media; picture-in-picture; web-share" allowFullScreen/></div>:hasUrl?<button className="reel-art" onClick={onPlay} aria-label={`Play ${video.title}`}>{thumbnail&&!failed?<img src={thumbnail} alt="" onError={()=>setFailed(true)}/>:<span className="plate">{video.category==="Travel"?"✈️":video.category==="Gardening"?"🌿":video.category==="Life Lessons"?"💡":video.category==="Food"?"🍲":"🎬"}</span>}<span className="play">▶</span></button>:<div className="reel-art" aria-label={`${video.title} manual item`}><span className="plate">{video.category==="Food"?"🍲":"📝"}</span></div>}
    <div className="card-body"><p className="eyebrow">{video.category}{video.subcategory?` · ${video.subcategory}`:""}</p><h2>{video.title}</h2><p className="saved-date">Saved {new Date(video.addedAt).toLocaleString()}</p>{video.tags.length?<div className="tags">{video.tags.map(t=><span key={t}>{t}</span>)}</div>:null}{video.notes?<p className="notes">{video.notes}</p>:<p className="notes muted">{hasUrl?"Add a note about this video.":"Manual menu/library item."}</p>}<div className="card-actions">{video.category==="Food"?(video.recipe?<button className="recipe-action" onClick={onRecipe}>🍳 Recipe</button>:hasUrl?<button className="recipe-action video-analysis" onClick={onAnalyzeVideo} disabled={videoAnalyzing}>{videoAnalyzing?"Analyzing…":analysisReady?"📋 View Analysis":"🎥 Analyze Video"}</button>:<button className="recipe-action" onClick={onManualRecipe}>✎ Add Recipe</button>):null}{hasUrl?<a href={video.url} target="_blank" rel="noreferrer" className="watch">{watchLabel}</a>:null}<button type="button" className="video-card-edit-action" onClick={e=>{e.preventDefault();e.stopPropagation();onEdit()}}>Edit</button><button onClick={onStatus}>{unavailable?"Restore":"Mark unavailable"}</button><button className="delete" onClick={onDelete}>Delete</button></div></div></article>
}
...
function WeeklyMenuThumb({item}:{item:WeeklyMenuItem}){
  const[thumbnail,setThumbnail]=useState<string|null>(null),[failed,setFailed]=useState(false);
  useEffect(()=>{let active=true;setThumbnail(null);setFailed(false);if(!hasExternalVideoUrl(item.url)){setFailed(true);return()=>{active=false}}fetch(`/api/thumbnail?url=${encodeURIComponent(item.url)}`).then(r=>r.ok?r.json():Promise.reject()).then((data:{thumbnail?:string})=>{if(active&&data.thumbnail)setThumbnail(data.thumbnail)}).catch(()=>{if(active)setFailed(true)});return()=>{active=false}},[item.url]);
  return <div className={`weekly-thumb ${thumbnail&&!failed?"has-image":""}`}>{thumbnail&&!failed?<img src={thumbnail} alt="" onError={()=>setFailed(true)}/>:<div className={`weekly-thumb-fallback ${hasExternalVideoUrl(item.url)?item.source.toLowerCase():"manual"}`}><span>{hasExternalVideoUrl(item.url)?"▶":"🍲"}</span><small>{hasExternalVideoUrl(item.url)?item.source:"Manual"}</small></div>}{hasExternalVideoUrl(item.url)?<span className="weekly-thumb-play">▶</span>:null}</div>
}
...
function WeeklyMenuPlanner({videos,onClose,onRecipeSaved,onOpenRecipe}:{videos:Video[];onClose:()=>void;onRecipeSaved:(videoId:string,recipe:Recipe)=>void;onOpenRecipe:(videoId:string)=>void}){
  const[weekStart,setWeekStart]=useState(currentMondayISO()),[items,setItems]=useState<WeeklyMenuItem[]>(()=>generateWeeklyItems(videos)),[savedMenus,setSavedMenus]=useState<SavedWeeklyMenu[]>([]),[activeSavedId,setActiveSavedId]=useState(""),[menuName,setMenuName]=useState(""),[status,setStatus]=useState(""),[loadingSaved,setLoadingSaved]=useState(true),[saving,setSaving]=useState(false),[savedMenuDirty,setSavedMenuDirty]=useState(false),[recipeCheckOpen,setRecipeCheckOpen]=useState(false),[batch,setBatch]=useState<BatchRecipeState>({running:false,total:0,completed:0,success:0,failed:0,currentIds:[],failures:[]}),[groceryLoading,setGroceryLoading]=useState(false),[groceryList,setGroceryList]=useState<GroceryListResult|null>(null),[groceryStage,setGroceryStage]=useState<"select"|"final">("select"),[groceryDayEnabled,setGroceryDayEnabled]=useState<Record<string,boolean>>(()=>Object.fromEntries(WEEKLY_MENU_DAYS.map(day=>[day,true]))),[groceryMenuEnabled,setGroceryMenuEnabled]=useState<Record<string,boolean>>({}),[selectedGroceryIngredients,setSelectedGroceryIngredients]=useState<Set<string>>(()=>new Set()),[replaceTarget,setReplaceTarget]=useState<{day:string;slot:WeeklyMenuSlot}|null>(null),[replaceQuery,setReplaceQuery]=useState(""),[manualRecipeVideoId,setManualRecipeVideoId]=useState<string|null>(null),[manualRecipeDraft,setManualRecipeDraft]=useState<RecipeDraft|null>(null),[manualRecipeSaving,setManualRecipeSaving]=useState(false),[singleRetryId,setSingleRetryId]=useState<string|null>(null);
  ...
  const groceryMenuKey=(day:string,slot:WeeklyMenuSlot)=>`${day}::${slot}`;
  const ingredientSelectionKey=(ingredient:string)=>canonicalGroceryItem(splitGroceryItemQuantity(ingredient).item)||normalizeGroceryIngredientText(ingredient).toLowerCase();
  const selectedWeeklyItems=()=>items.filter(item=>(groceryDayEnabled[item.day]??true)&&(groceryMenuEnabled[groceryMenuKey(item.day,item.slot)]??true));
  const searchableFoodVideos=useMemo(()=>{
    const q=replaceQuery.trim().toLowerCase();
    return videos
      .filter(video=>video.status==="available"&&video.category.toLowerCase()==="food")
      .filter(video=>!q||[video.title,video.subcategory,video.notes,...video.tags].join(" ").toLowerCase().includes(q))
      .sort((a,b)=>{
        if(replaceTarget){const am=matchesWeeklySlot(a,replaceTarget.slot)?1:0,bm=matchesWeeklySlot(b,replaceTarget.slot)?1:0;if(am!==bm)return bm-am}
        return a.title.localeCompare(b.title);
      })
      .slice(0,40);
  },[videos,replaceQuery,replaceTarget]);
  function resetWeeklyGrocerySelections(){setGroceryDayEnabled(Object.fromEntries(WEEKLY_MENU_DAYS.map(day=>[day,true])));setGroceryMenuEnabled({});setSelectedGroceryIngredients(new Set());setGroceryStage("select");setGroceryList(null)}
  function markMenuChanged(message:string){if(activeSavedId){setSavedMenuDirty(true);setStatus(`${message} Use Overwrite Saved Menu to update the saved menu.`)}else setStatus(message)}
  function replaceWithKnownMenu(video:Video){if(!replaceTarget)return;const{day,slot}=replaceTarget;const next=menuItemFromVideo(day,slot,video);setItems(existing=>[...existing.filter(item=>!(item.day===day&&item.slot===slot)),next]);setReplaceTarget(null);setReplaceQuery("");resetDerived();markMenuChanged(`${slot} replaced with ${video.title} for ${day}.`)}
  function toggleGroceryIngredient(ingredient:string,checked:boolean){const key=ingredientSelectionKey(ingredient);if(!key)return;setSelectedGroceryIngredients(current=>{const next=new Set(current);if(checked)next.add(key);else next.delete(key);return next})}

  function resetDerived(){batchStopRef.current=true;batchRunIdRef.current+=1;batchRunningRef.current=false;batchCompletedIdsRef.current=new Set();setRecipeCheckOpen(false);setGroceryList(null);setGroceryStage("select");setSelectedGroceryIngredients(new Set());setBatch({running:false,total:0,completed:0,success:0,failed:0,currentIds:[],failures:[]})}
  function regenerate(){setItems(generateWeeklyItems(videos));setActiveSavedId("");setMenuName("");setSavedMenuDirty(false);resetDerived();resetWeeklyGrocerySelections();setStatus(foodCount?"New weekly menu generated.":"Add Food videos before generating a weekly menu.")}
  function moveWeek(delta:number){setWeekStart(current=>shiftWeekISO(current,delta));setItems(generateWeeklyItems(videos));setActiveSavedId("");setMenuName("");setSavedMenuDirty(false);resetDerived();resetWeeklyGrocerySelections()}
  function reroll(day:string,slot:WeeklyMenuSlot){const current=items.find(item=>item.day===day&&item.slot===slot),used=new Set(items.filter(item=>item!==current).map(item=>item.videoId));const video=chooseWeeklyVideo(videos,slot,used);if(!video){setStatus(`No matching Food videos are available for ${slot}.`);return}const next=menuItemFromVideo(day,slot,video);setItems(existing=>[...existing.filter(item=>!(item.day===day&&item.slot===slot)),next]);resetDerived();markMenuChanged(`${slot} replaced randomly for ${day}.`)}
  ...
  function loadMenu(id:string){const menu=savedMenus.find(item=>item.id===id);if(!menu)return;setActiveSavedId(menu.id);setMenuName(menu.name);setWeekStart(menu.weekStart);setItems(menu.items);setSavedMenuDirty(false);resetDerived();resetWeeklyGrocerySelections();setStatus(`Loaded ${menu.name}.`)}
  ...
  async function retryDescriptionIntoWeeklyManualEditor(){
    const video=videos.find(item=>item.id===manualRecipeVideoId);
    if(!video||!manualRecipeDraft||!hasExternalVideoUrl(video.url)||singleRetryId)return;
    const mergeLines=(existing:string,incoming:string[])=>{
      const rows=existing.split("\n").map(item=>item.trim()).filter(Boolean);
      const seen=new Set(rows.map(item=>item.toLowerCase().replace(/[^a-z0-9¼½¾⅓⅔⅛⅜⅝⅞]+/gi," ").trim()));
      for(const raw of incoming){
        const item=raw.trim();
        if(!item)continue;
        const key=item.toLowerCase().replace(/[^a-z0-9¼½¾⅓⅔⅛⅜⅝⅞]+/gi," ").trim();
        if(!key||seen.has(key))continue;
        seen.add(key);rows.push(item);
      }
      return rows.join("\n");
    };
    const mergeNotes=(existing:string,incoming:string)=>{
      const current=existing.trim(),next=incoming.trim();
      if(!next)return current;if(!current)return next;
      const a=current.toLowerCase(),b=next.toLowerCase();
      if(a.includes(b))return current;if(b.includes(a))return next;
      return `${current}\n\n${next}`;
    };
    setSingleRetryId(video.id);
    setStatus(`Reading the saved description for ${video.title}…`);
    try{
      const response=await fetch("/api/recipe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({video:{url:video.url,title:video.title,notes:video.notes},mode:"max"})});
      const result=await response.json() as {recipe?:Recipe|null;descriptionFound?:boolean;partial?:boolean;message?:string};
      if(!response.ok)throw new Error(result.message||`Description retry returned HTTP ${response.status}`);
      if(!result.recipe){setStatus(result.descriptionFound?"Description was found, but it did not contain usable recipe details.":"No usable public description was available.");return}
      const candidate=result.recipe;
      setManualRecipeDraft(current=>current?{
        title:!current.title.trim()||isGenericTitle(current.title)?candidate.title||current.title:current.title,
        servings:current.servings.trim()||candidate.servings||"",
        prepTime:current.prepTime.trim()||candidate.prepTime||"",
        ingredients:mergeLines(current.ingredients,candidate.ingredients||[]),
        steps:mergeLines(current.steps,candidate.steps||[]),
        notes:mergeNotes(current.notes,candidate.notes||"")
      }:current);
      setStatus(result.partial?"Description details merged into the manual recipe form. Some recipe details are still missing.":"Maximum reliable description details merged into the manual recipe form.");
    }catch(error){setStatus(error instanceof Error?error.message:"Could not retry the recipe description")}finally{setSingleRetryId(null)}
  }
  ...
  function openGrocerySelection(){
    if(!activeSavedId){setStatus("Save the weekly menu first.");return}
    const selected=selectedWeeklyItems();
    if(!selected.length){setStatus("Select at least one day or menu item for the Grocery List.");return}
    const menuItems:GroceryListMenuItem[]=selected.map(item=>{
    ...
    setSelectedGroceryIngredients(new Set());
    setGroceryStage("select");
    setGroceryList({menuItems,summary:[],aisleGroups:[],generatedAt:new Date().toISOString()});
    const missing=menuItems.filter(item=>item.recipeMissing).length;
    setStatus(missing?`Review ${menuItems.length} selected menus. ${missing} ${missing===1?"is":"are"} missing a recipe.`:`Review ${menuItems.length} selected menus and choose the ingredients you need.`);
  }
  async function prepareFinalGroceryList(){
    if(!groceryList)return;
    if(!selectedGroceryIngredients.size){setStatus("Check at least one ingredient before preparing the final Grocery List.");return}
    setGroceryLoading(true);
    try{
      const menuItems:GroceryListMenuItem[]=groceryList.menuItems.map(item=>({...item,ingredients:item.recipeMissing?[]:item.ingredients.filter(ingredient=>selectedGroceryIngredients.has(ingredientSelectionKey(ingredient)))}));
      ...
      const aisleGroups=Array.isArray(result.aisleGroups)&&result.aisleGroups.length?result.aisleGroups:groupGrocerySummaryByAisle(summary);
      setGroceryList(current=>current?{...current,summary,aisleGroups,generatedAt:result.generatedAt||new Date().toISOString()}:current);
      setGroceryStage("final");
      setStatus("Final Grocery List prepared from the ingredients you selected.");
    }catch{setStatus("Could not prepare the final Grocery List. Please try again.")}finally{setGroceryLoading(false)}
  }
  async function copyGroceryList(){
    if(!groceryList||groceryStage!=="final")return;
    const text=["Final Grocery List","",...groceryList.aisleGroups.flatMap(group=>[group.aisle,...group.items.map(item=>`  • ${item.item}${item.quantity?` — ${item.quantity}`:""}`),""])].join("\n");
    try{await navigator.clipboard.writeText(text);setStatus("Final Grocery List copied.")}catch{setStatus("Could not copy the Grocery List.")}
  }
  function exportGroceryListExcel(){
    if(!groceryList||groceryStage!=="final")return;
    const xmlEscape=(value:string)=>value.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\"/g,"&quot;").replace(/'/g,"&apos;");
    const cell=(value:string)=>`<Cell><Data ss:Type="String">${xmlEscape(value)}</Data></Cell>`;
    const summaryRows=groceryList.aisleGroups.flatMap(group=>group.items.map(item=>`<Row>${cell(group.aisle)}${cell(item.item)}${cell(item.quantity||"")}${cell(item.usedBy.join(", "))}</Row>`)).join("");
    const menuRows=groceryList.menuItems.map(item=>`<Row>${cell(item.schedule)}${cell(item.title)}${cell(item.recipeMissing?"Recipe missing":item.ingredients.filter(ingredient=>selectedGroceryIngredients.has(ingredientSelectionKey(ingredient))).join(" | "))}</Row>`).join("");
    const xml=`<?xml version="1.0"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="Final Grocery List"><Table><Row>${cell("Aisle")}${cell("Item")}${cell("Quantity")}${cell("Used By")}</Row>${summaryRows}</Table></Worksheet><Worksheet ss:Name="Selected Menus"><Table><Row>${cell("Schedule")}${cell("Menu Item")}${cell("Selected Ingredients")}</Row>${menuRows}</Table></Worksheet></Workbook>`;
    const blob=new Blob([xml],{type:"application/vnd.ms-excel"}),a=document.createElement("a");
    const safe=(menuName||`weekly-grocery-${weekStart}`).replace(/[^a-z0-9-_]+/gi,"-").replace(/^-+|-+$/g,"")||"weekly-grocery-list";
    a.href=URL.createObjectURL(blob);a.download=`${safe}.xls`;a.click();URL.revokeObjectURL(a.href);setStatus("Grocery List exported to Excel.");
  }
  ...
  return <div className="dialog weekly-menu-dialog">
    ...
    {!foodCount?<div className="weekly-menu-empty"><strong>No Food videos yet</strong><span>Classify or move videos into Food, then generate the weekly menu.</span></div>:<div className="weekly-menu-scroll"><div className="weekly-calendar weekly-calendar-matrix"><div className="weekly-calendar-header-row"><div className="weekly-day-column-title"><span>Day</span></div>{WEEKLY_MENU_SLOTS.map(slot=><div className="weekly-meal-column-title" key={slot}><span>{slot}</span></div>)}</div>{WEEKLY_MENU_DAYS.map((day,dayIndex)=>{const dayIncluded=groceryDayEnabled[day]??true;return <div className="weekly-calendar-row" key={day}><div className="weekly-day-cell"><label style={{display:"flex",gap:6,alignItems:"center",cursor:"pointer"}}><input type="checkbox" checked={dayIncluded} onChange={e=>{setGroceryDayEnabled(current=>({...current,[day]:e.target.checked}));setGroceryList(null)}}/><strong>{day}</strong></label><small>{dayDateLabel(weekStart,dayIndex)}</small><small>{dayIncluded?"Included in Grocery List":"Excluded from Grocery List"}</small></div>{WEEKLY_MENU_SLOTS.map(slot=>{const item=items.find(menuItem=>menuItem.day===day&&menuItem.slot===slot),liveVideo=item?videos.find(v=>v.id===item.videoId):null,key=groceryMenuKey(day,slot),mealIncluded=dayIncluded&&(groceryMenuEnabled[key]??true);return <div className="weekly-meal-cell" key={slot} style={{position:"relative"}}><div style={{display:"flex",gap:4,justifyContent:"flex-end",alignItems:"center",marginBottom:4}}><button type="button" title={`Choose another ${slot} randomly`} aria-label={`Choose another ${slot} randomly for ${day}`} onClick={()=>reroll(day,slot)}>↻</button><button type="button" title={`Search saved Food items for ${slot}`} aria-label={`Search saved Food items for ${day} ${slot}`} onClick={()=>{setReplaceTarget({day,slot});setReplaceQuery("")}}>⌕</button></div>{item?<div className="weekly-video-card-shell"><label style={{display:"flex",gap:6,alignItems:"center",fontSize:12,marginBottom:4,cursor:dayIncluded?"pointer":"not-allowed",opacity:dayIncluded?1:.55}}><input type="checkbox" checked={mealIncluded} disabled={!dayIncluded} onChange={e=>{setGroceryMenuEnabled(current=>({...current,[key]:e.target.checked}));setGroceryList(null)}}/><span>Include in Grocery List</span></label>{hasExternalVideoUrl(item.url)?<a className="weekly-video-card" href={item.url} target="_blank" rel="noreferrer"><WeeklyMenuThumb item={item}/><div className="weekly-video-card-copy"><strong>{item.title}</strong><small>{item.subcategory||item.source}</small></div></a>:<div className="weekly-video-card"><WeeklyMenuThumb item={item}/><div className="weekly-video-card-copy"><strong>{item.title}</strong><small>{item.subcategory||"Manual Food item"}</small></div></div>}{liveVideo?.recipe?<button type="button" className="weekly-recipe-note" title="View recipe" aria-label={`View recipe for ${item.title}`} onClick={()=>onOpenRecipe(item.videoId)}>🗒</button>:liveVideo?<button type="button" className="weekly-manual-recipe" title="Add recipe manually" aria-label={`Add recipe manually for ${item.title}`} onClick={()=>openWeeklyRecipeEditor(item.videoId)}>✎</button>:null}</div>:<div className="weekly-no-video"><span>No matching</span><strong>{slot}</strong></div>}</div>})}</div>})}</div></div>}
    ...
    {replaceTarget?<div className="weekly-manual-recipe-backdrop" style={{zIndex:10350}} onMouseDown={e=>{if(e.target===e.currentTarget){setReplaceTarget(null);setReplaceQuery("")}}}><div className="weekly-manual-recipe-panel" style={{maxWidth:760}}><button className="dialog-close" onClick={()=>{setReplaceTarget(null);setReplaceQuery("")}}>×</button><p className="kicker">REPLACE MENU ITEM</p><h2>Search a saved Food item</h2><p>Replace {replaceTarget.day} · {replaceTarget.slot} with any known Food item. Items matching the meal type are shown first.</p><label>Search<input autoFocus value={replaceQuery} onChange={e=>setReplaceQuery(e.target.value)} placeholder="Search dish name, subcategory, notes, or tags"/></label><div style={{display:"grid",gap:8,maxHeight:"55vh",overflowY:"auto",marginTop:14}}>{searchableFoodVideos.length?searchableFoodVideos.map(video=><button type="button" key={video.id} onClick={()=>replaceWithKnownMenu(video)} style={{display:"flex",justifyContent:"space-between",gap:12,textAlign:"left",padding:12}}><span><strong>{video.title}</strong><small style={{display:"block"}}>{video.subcategory||"Food"}{video.recipe?" · Recipe saved":" · Recipe missing"}</small></span><span>{matchesWeeklySlot(video,replaceTarget.slot)?"Suggested":"Choose"}</span></button>):<p>No matching Food items found.</p>}</div><div className="analysis-actions"><button type="button" onClick={()=>{setReplaceTarget(null);setReplaceQuery("")}}>Cancel</button></div></div></div>:null}
    {manualRecipeVideoId&&manualRecipeDraft?<div className="weekly-manual-recipe-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)closeWeeklyRecipeEditor()}}><div className="weekly-manual-recipe-panel"><button className="dialog-close" onClick={closeWeeklyRecipeEditor}>×</button><p className="kicker">{videos.find(video=>video.id===manualRecipeVideoId)?.recipe?"EDIT RECIPE":"ADD RECIPE MANUALLY"}</p><h2>{videos.find(video=>video.id===manualRecipeVideoId)?.title??"Recipe"}</h2><p className="weekly-manual-recipe-intro">Enter or correct the recipe here. Saving returns you to the same Weekly Menu and immediately updates recipe readiness.</p>{hasExternalVideoUrl(videos.find(video=>video.id===manualRecipeVideoId)?.url??"")?<div className="recipe-footer recipe-editor-retry-bar"><button type="button" className="primary" onClick={()=>void retryDescriptionIntoWeeklyManualEditor()} disabled={singleRetryId===manualRecipeVideoId}>{singleRetryId===manualRecipeVideoId?"Retrying description…":"↻ Retry from Description"}</button><small className="recipe-disclaimer">Use the saved video description to prefill the maximum reliable recipe details. Review everything before saving.</small></div>:<div className="recipe-footer recipe-editor-retry-bar"><button type="button" disabled>↻ Retry from Description — no video link</button></div>}<div className="recipe-edit-form"><label>Recipe title<input value={manualRecipeDraft.title} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,title:e.target.value})}/></label><div className="form-row"><label>Servings<input value={manualRecipeDraft.servings} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,servings:e.target.value})}/></label><label>Prep / cooking time<input value={manualRecipeDraft.prepTime} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,prepTime:e.target.value})}/></label></div><label>Ingredients <small>One ingredient per line · required for Grocery List</small><textarea rows={10} value={manualRecipeDraft.ingredients} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,ingredients:e.target.value})} placeholder={"1 lb chicken\n1 cup yogurt\n2 tbsp spice mix"}/></label><label>Steps <small>One step per line</small><textarea rows={9} value={manualRecipeDraft.steps} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,steps:e.target.value})} placeholder={"Marinate the chicken\nCook until done\nServe warm"}/></label><label>Notes<textarea rows={4} value={manualRecipeDraft.notes} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,notes:e.target.value})}/></label><div className="analysis-actions"><button type="button" className="primary" onClick={saveWeeklyManualRecipe} disabled={manualRecipeSaving}>{manualRecipeSaving?"Saving…":"Save Recipe"}</button><button type="button" onClick={closeWeeklyRecipeEditor}>Cancel</button><a href={videos.find(video=>video.id===manualRecipeVideoId)?.url} target="_blank" rel="noreferrer">Open original ↗</a></div></div></div></div>:null}
    {groceryList?<div className="weekly-grocery-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setGroceryList(null)}}><div className="weekly-grocery-panel"><button className="dialog-close" onClick={()=>setGroceryList(null)}>×</button><p className="kicker">{groceryStage==="select"?"SELECT GROCERIES":"FINAL GROCERY LIST"}</p><h2>{menuName||`Week of ${weekTitle(weekStart)}`}</h2>{groceryStage==="select"?<><p className="weekly-grocery-intro">These are the menu items you selected in Weekly Menu. Check only the ingredients you need to buy. The same ingredient is synchronized across every menu automatically.</p><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}><button type="button" onClick={()=>{const keys=new Set<string>();for(const item of groceryList.menuItems)for(const ingredient of item.ingredients){const key=ingredientSelectionKey(ingredient);if(key)keys.add(key)}setSelectedGroceryIngredients(keys)}}>Select all ingredients</button><button type="button" onClick={()=>setSelectedGroceryIngredients(new Set())}>Clear all</button><span style={{alignSelf:"center"}}>{selectedGroceryIngredients.size} grocery item{selectedGroceryIngredients.size===1?"":"s"} selected</span></div><div className="weekly-grocery-items">{groceryList.menuItems.map((item,index)=><section key={`${item.videoId}-${index}`}><div><strong>{item.schedule}</strong><span>{item.title}</span></div>{item.recipeMissing?<p><strong>Recipe missing</strong> — ingredients unavailable</p>:<ul style={{listStyle:"none",paddingLeft:0}}>{item.ingredients.map((ingredient,i)=>{const key=ingredientSelectionKey(ingredient),checked=Boolean(key)&&selectedGroceryIngredients.has(key);return <li key={`${i}-${ingredient}`}><label style={{display:"flex",gap:8,alignItems:"flex-start",cursor:"pointer"}}><input type="checkbox" checked={checked} onChange={e=>toggleGroceryIngredient(ingredient,e.target.checked)}/><span>{ingredient}</span></label></li>})}</ul>}</section>)}</div><div className="analysis-actions"><button type="button" className="primary" onClick={()=>void prepareFinalGroceryList()} disabled={groceryLoading||!selectedGroceryIngredients.size}>{groceryLoading?"Preparing…":"Prepare Final Grocery List"}</button><button type="button" onClick={()=>setGroceryList(null)}>Back to Weekly Menu</button></div></>:<><p className="weekly-grocery-intro">Only the ingredients you checked are included below, consolidated and ordered by grocery aisle.</p><section className="weekly-grocery-summary"><h3>Shopping summary by aisle</h3>{groceryList.aisleGroups.length?groceryList.aisleGroups.map(group=><div key={group.aisle}><h4>{group.aisle}</h4><ul>{group.items.map((item,index)=><li key={`${group.aisle}-${index}-${item.item}`}><div><strong>{item.item}</strong>{item.quantity?<span>{item.quantity}</span>:null}</div>{item.usedBy.length?<small>Used by: {item.usedBy.join(", ")}</small>:null}</li>)}</ul></div>):<p>No selected ingredients were available for the final list.</p>}</section><div className="analysis-actions"><button type="button" className="primary" onClick={exportGroceryListExcel}>Export Excel</button><button type="button" onClick={()=>void copyGroceryList()}>Copy Grocery List</button><button type="button" onClick={()=>setGroceryStage("select")}>Back to Ingredient Selection</button><button type="button" onClick={()=>setGroceryList(null)}>Close</button></div></>}</div></div>:null}
  </div>
}
...
export default function Home(){
  ...
  const[manualRecipeVideoId,setManualRecipeVideoId]=useState<string|null>(null),[manualRecipeDraft,setManualRecipeDraft]=useState<RecipeDraft|null>(null);
  ...
  useEffect(()=>{if(!ready)return;localStorage.setItem(STORAGE_KEY,JSON.stringify(videos));localStorage.setItem(CATEGORY_KEY,JSON.stringify(categories));const timer=window.setTimeout(()=>{if(libraryWriteInProgressRef.current)return;fetch("/api/library",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({videos,categories})}).then(response=>{if(response.status===401)location.href="/sign-in"}).catch(()=>{})},500);return()=>window.clearTimeout(timer)},[videos,categories,ready]);useEffect(()=>{if(!toast)return;const t=setTimeout(()=>setToast(""),3200);return()=>clearTimeout(t)},[toast]);
  ...
  useEffect(()=>{if(!ready)return;let refreshing=false;const refreshSharedVideos=async()=>{if(refreshing||document.visibilityState!=="visible"||libraryWriteInProgressRef.current)return;const revisionAtStart=libraryMutationRef.current;refreshing=true;try{const response=await fetch("/api/library",{cache:"no-store"});if(response.status===401){location.href="/sign-in";return}if(!response.ok)return;const result=await response.json() as {exists:boolean;data?:{videos?:Partial<Video>[];categories?:Category[]}};if(libraryWriteInProgressRef.current||revisionAtStart!==libraryMutationRef.current)return;if(result.exists&&result.data){setVideos(normalizeStoredVideos(result.data.videos??[]));if(result.data.categories?.length)setCategories(result.data.categories)}}finally{refreshing=false}};const onVisible=()=>{if(document.visibilityState==="visible")void refreshSharedVideos()};document.addEventListener("visibilitychange",onVisible);window.addEventListener("focus",onVisible);void refreshSharedVideos();return()=>{document.removeEventListener("visibilitychange",onVisible);window.removeEventListener("focus",onVisible)}},[ready]);
  ...
  async function persistLibraryNow(nextVideos:Video[],verifyVideoId:string){
    ...
    try{
      const response=await fetch("/api/library",{
        method:"PUT",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({videos:nextVideos,categories})
      });
      if(response.status===401){location.href="/sign-in";return false}
      if(!response.ok){
        const detail=(await response.text()).slice(0,300);
        throw new Error(`Library save returned HTTP ${response.status}${detail?`: ${detail}`:""}`);
      }

      // Verify against the database when possible. This catches a successful HTTP
      // response that did not actually retain the newly-added/manual item.
      try{
        const verifyResponse=await fetch("/api/library",{cache:"no-store"});
        if(verifyResponse.ok){
          const verifyResult=await verifyResponse.json() as {exists:boolean;data?:{videos?:Partial<Video>[]}};
          const stored=verifyResult.data?.videos??[];
          if(!stored.some(item=>item.id===verifyVideoId)){
            throw new Error("The database responded successfully but the saved item was not returned by the library read-back.");
          }
        }
      }catch(error){
        if(error instanceof Error&&error.message.includes("saved item was not returned"))throw error;
        console.warn("Library save succeeded but read-back verification was unavailable",error);
      }

      return true;
    }catch(error){
      console.error("Could not persist ReelRecall library item",error);
      setToast(error instanceof Error?`Database save failed: ${error.message}`:"Database save failed. The item was not saved.");
      return false;
    }finally{
      if(revision===libraryMutationRef.current){
  ...
  async function saveVideo(e:FormEvent){
    ...
    if(!title){setToast("Add a title before saving");return}
    const rawUrl=form.url.trim();
    const normalizedInput=rawUrl&&!/^https?:\/\//i.test(rawUrl)?`https://${rawUrl}`:rawUrl;
    const externalUrl=normalizedInput?cleanUrl(normalizedInput):"";
    const tags=form.tags.split(",").map(t=>t.trim()).filter(Boolean);
    if(externalUrl&&videos.some(v=>v.id!==editingId&&hasExternalVideoUrl(v.url)&&canonicalUrl(v.url)===canonicalUrl(externalUrl))){
      setToast("That video is already in your library");
      return;
    }

    let verifyVideoId="";
    let nextVideos:Video[];
    let savedExternalUrl=externalUrl;
    if(editingId){
      verifyVideoId=editingId;
      const storageUrl=externalUrl||manualItemStorageUrl(editingId);
      savedExternalUrl=externalUrl;
      nextVideos=videos.map(v=>v.id===editingId?{...v,...form,title,url:storageUrl,tags,source:externalUrl?sourceFor(externalUrl):"Other",titleLocked:true,categoryLocked:true,tagsLocked:true,aiStatus:"done"}:v);
    }else{
      const id=crypto.randomUUID();
      const storageUrl=externalUrl||manualItemStorageUrl(id);
      const item:Video={id,...form,title,url:storageUrl,tags,source:externalUrl?sourceFor(externalUrl):"Other",favorite:false,addedAt:new Date().toISOString(),titleLocked:true,categoryLocked:true,tagsLocked:true,aiStatus:"done",recipe:null};
      verifyVideoId=item.id;
      nextVideos=[item,...videos];
    }

    const persisted=await persistLibraryNow(nextVideos,verifyVideoId);
    if(!persisted)return;

    setVideos(nextVideos);
    localStorage.setItem(STORAGE_KEY,JSON.stringify(nextVideos));
    localStorage.setItem(CATEGORY_KEY,JSON.stringify(categories));
    setToast(editingId?(savedExternalUrl?"Item updated in database":"Manual item updated in database"):(savedExternalUrl?"Item saved to database":"Manual item saved to database"));
    setDialogOpen(false);
  }
  ...
  async function importFile(e:ChangeEvent<HTMLInputElement>){
    ...
    try{
      if(file.name.toLowerCase().endsWith(".json")){
        const imported=normalizeStoredVideos(JSON.parse(await file.text()) as Partial<Video>[]);
        const seenUrls=new Set(videos.filter(v=>hasExternalVideoUrl(v.url)).map(v=>canonicalUrl(v.url)));
        const seenIds=new Set(videos.map(v=>v.id));
        const unique=imported.filter(v=>{
          if(hasExternalVideoUrl(v.url))return!seenUrls.has(canonicalUrl(v.url));
          return!seenIds.has(v.id);
        });
        setVideos(a=>[...unique,...a]);
        setToast(`Imported ${unique.length} library items from backup`);
      }else{
        const added=extractVideos(await file.text(),videos);
        setVideos(a=>[...added,...a]);
        setToast(added.length?`Imported ${added.length} new links. Organizing…`:"No new links—duplicates and prior corrections were preserved");
        if(added.length)void autoCategorize(added);
      }
    }catch{setToast("Could not read that file")}
    e.target.value="";
  }
  async function extractRecipe(video:Video){if(video.recipe){setRecipeVideoId(video.id);return}setRecipeLoadingId(video.id);try{const response=await fetch("/api/recipe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({video:{url:video.url,title:video.title,notes:video.notes}})});if(!response.ok)throw new Error();const result=await response.json() as {recipe:Recipe|null;descriptionFound?:boolean;facebookBlocked?:boolean;partial?:boolean;missing?:string[]};if(!result.recipe){if(video.source==="Facebook"){setRecipePasteVideoId(video.id);setRecipePasteText("");setToast(result.descriptionFound?"Facebook exposed text, but no reliable recipe ingredient or cooking detail was found":"Facebook did not expose the reel description to ReelRecall")}else setToast(result.descriptionFound?"The reel description did not contain a usable recipe ingredient or cooking detail":"No usable recipe description was available for this reel");return}setVideos(current=>current.map(item=>item.id===video.id?applyRecipeCandidate(item,{...(result.recipe as Recipe),source:"description"},"description",true):item));setRecipeVideoId(video.id);setToast(result.partial?`Partial recipe saved${result.missing?.length?` · missing ${result.missing.join(" and ")}`:""}`:"Recipe extracted and saved")}catch{setToast("Recipe extraction is unavailable. Check the OpenAI API configuration")}finally{setRecipeLoadingId(null)}}
  async function extractPastedRecipe(){const video=recipePasteVideoId?videos.find(v=>v.id===recipePasteVideoId):null;if(!video||!recipePasteText.trim())return;setRecipeLoadingId(video.id);try{const response=await fetch("/api/recipe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({video:{url:video.url,title:video.title,notes:video.notes},description:recipePasteText.trim()})});if(!response.ok)throw new Error();const result=await response.json() as {recipe:Recipe|null;partial?:boolean;missing?:string[]};if(!result.recipe){setToast("The pasted description did not contain a reliable ingredient or cooking detail");return}setVideos(current=>current.map(item=>item.id===video.id?applyRecipeCandidate(item,{...(result.recipe as Recipe),source:"description"},"description",true):item));setRecipePasteVideoId(null);setRecipePasteText("");setRecipeVideoId(video.id);setToast(result.partial?`Partial recipe extracted${result.missing?.length?` · missing ${result.missing.join(" and ")}`:""}`:"Recipe extracted and saved")}catch{setToast("Recipe extraction is unavailable. Check the OpenAI API configuration")}finally{setRecipeLoadingId(null)}}
  async function refreshRecipe(video:Video){setRecipeLoadingId(video.id);try{const response=await fetch("/api/recipe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({video:{url:video.url,title:video.title,notes:video.notes},mode:"max"})});if(!response.ok)throw new Error();const result=await response.json() as {recipe:Recipe|null;descriptionFound?:boolean;partial?:boolean;missing?:string[]};if(!result.recipe){setToast(result.descriptionFound?"The reel description did not contain a usable ingredient or cooking detail":"No usable recipe description was available for this reel");return}const candidate={...(result.recipe as Recipe),source:"description" as const};if(video.recipe?.source==="manual"){setToast("Manual recipe protected. Use Edit Recipe → Retry from Description to merge fetched details.");return}if(video.recipe&&!descriptionRecipeIsRicher(video.recipe,candidate)){setToast("The saved recipe is already as detailed as the description result");return}setVideos(current=>current.map(item=>item.id===video.id?applyRecipeCandidate(item,candidate,"description",false):item));setToast(result.partial?`Partial recipe refreshed${result.missing?.length?` · missing ${result.missing.join(" and ")}`:""}`:"Recipe refreshed from description")}catch{setToast("Recipe extraction is unavailable. Check the OpenAI API configuration")}finally{setRecipeLoadingId(null)}}
  ...
  function mergeRecipeDraftLines(existing:string,incoming:string[]){
    const rows=existing.split("\n").map(item=>item.trim()).filter(Boolean);
    const seen=new Set(rows.map(item=>item.toLowerCase().replace(/[^a-z0-9¼½¾⅓⅔⅛⅜⅝⅞]+/gi," ").trim()));
    for(const raw of incoming){
      const item=raw.trim();
  ...
  function mergeRecipeDraftNotes(existing:string,incoming:string){
    const current=existing.trim(),next=incoming.trim();
    if(!next)return current;if(!current)return next;
    const a=current.toLowerCase(),b=next.toLowerCase();
    if(a.includes(b))return current;if(b.includes(a))return next;
  ...
  async function retryDescriptionIntoEditor(video:Video){
    if(!recipeDraft||!hasExternalVideoUrl(video.url))return;
    setRecipeLoadingId(video.id);
    try{
      const response=await fetch("/api/recipe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({video:{url:video.url,title:video.title,notes:video.notes},mode:"max"})});
      const result=await response.json() as {recipe?:Recipe|null;descriptionFound?:boolean;partial?:boolean;missing?:string[];message?:string};
      if(!response.ok)throw new Error(result.message||`Description retry returned HTTP ${response.status}`);
      if(!result.recipe){setToast(result.descriptionFound?"Description was found, but it did not contain a usable ingredient or cooking detail":"No usable public description was available");return}
      const candidate=result.recipe;
      setRecipeDraft(current=>current?{
        title:!current.title.trim()||isGenericTitle(current.title)?candidate.title||current.title:current.title,
        servings:current.servings.trim()||candidate.servings||"",
        prepTime:current.prepTime.trim()||candidate.prepTime||"",
        ingredients:mergeRecipeDraftLines(current.ingredients,candidate.ingredients||[]),
        steps:mergeRecipeDraftLines(current.steps,candidate.steps||[]),
        notes:mergeRecipeDraftNotes(current.notes,candidate.notes||"")
      }:current);
      const detail=`${candidate.ingredients.length} ingredient${candidate.ingredients.length===1?"":"s"} / ${candidate.steps.length} step${candidate.steps.length===1?"":"s"}`;
      setToast(result.partial?`Description retry found ${detail}; partial recipe merged into the editor`:`Description retry found ${detail}; review the merged details before saving`);
    }catch(error){setToast(error instanceof Error?error.message:"Could not retry the recipe description")}finally{setRecipeLoadingId(null)}
  }
  function saveRecipeEdits(video:Video){if(!video.recipe||!recipeDraft)return;const ingredients=recipeDraft.ingredients.split("\n").map(item=>item.trim()).filter(Boolean),steps=recipeDraft.steps.split("\n").map(item=>item.trim()).filter(Boolean);if(!ingredients.length&&!steps.length){setToast("Recipe needs at least one ingredient or one preparation step");return}const updated:Recipe={...video.recipe,source:"manual",title:recipeDraft.title.trim()||video.title,servings:recipeDraft.servings.trim(),prepTime:recipeDraft.prepTime.trim(),ingredients,steps,notes:recipeDraft.notes.trim()};setVideos(current=>current.map(item=>item.id===video.id?applyRecipeCandidate(item,updated,"manual",true):item));setRecipeEditing(false);setRecipeDraft(null);setToast(steps.length?"Recipe changes saved":"Partial recipe saved · cooking steps are still missing")}
  async function retryDescriptionIntoManualEditor(){
    const video=manualRecipeVideoId?videos.find(v=>v.id===manualRecipeVideoId):null;
    if(!video||!manualRecipeDraft||!hasExternalVideoUrl(video.url))return;
    setRecipeLoadingId(video.id);
    try{
      const response=await fetch("/api/recipe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({video:{url:video.url,title:video.title,notes:video.notes},mode:"max"})});
      const result=await response.json() as {recipe?:Recipe|null;descriptionFound?:boolean;partial?:boolean;message?:string};
      if(!response.ok)throw new Error(result.message||`Description retry returned HTTP ${response.status}`);
      if(!result.recipe){setToast(result.descriptionFound?"Description was found, but it did not contain usable recipe details":"No usable public description was available");return}
      const candidate=result.recipe;
      setManualRecipeDraft(current=>current?{
        title:!current.title.trim()||isGenericTitle(current.title)?candidate.title||current.title:current.title,
        servings:current.servings.trim()||candidate.servings||"",
      ...
      const detail=`${candidate.ingredients.length} ingredient${candidate.ingredients.length===1?"":"s"} / ${candidate.steps.length} step${candidate.steps.length===1?"":"s"}`;
      setToast(result.partial?`Description retry found ${detail}; partial details merged into Add Recipe`:`Description retry found ${detail}; review the fields before saving`);
    }catch(error){setToast(error instanceof Error?error.message:"Could not retry the recipe description")}finally{setRecipeLoadingId(null)}
  }
  function openManualRecipe(video:Video){
    setManualRecipeVideoId(video.id);
    setManualRecipeDraft({
      title:video.recipe?.title||video.title,
      servings:video.recipe?.servings||"",
      prepTime:video.recipe?.prepTime||"",
      ingredients:(video.recipe?.ingredients??[]).join("\n"),
      steps:(video.recipe?.steps??[]).join("\n"),
      notes:video.recipe?.notes||""
    });
  }
  function closeManualRecipe(){
    setManualRecipeVideoId(null);
    setManualRecipeDraft(null);
  }
  function saveManualRecipe(){
    ...
    const steps=manualRecipeDraft.steps.split("\n").map(item=>item.trim()).filter(Boolean);
    if(!ingredients.length){setToast("Add at least one ingredient before saving the recipe");return}
    const recipe:Recipe={
      source:"manual",
      title:manualRecipeDraft.title.trim()||video.title,
      ingredients,
    ...
    setVideos(current=>current.map(item=>item.id===video.id?applyRecipeCandidate(item,recipe,"manual",true):item));
    closeManualRecipe();
    setRecipeVideoId(video.id);
    setToast("Manual recipe saved");
  }
...
try{result=await response.json()}catch{result={error:`Analysis service returned HTTP ${response.status}`}}const analysis=result.analysis;const status:ReelAnalysisResult["status"]=analysis?.status??(response.ok?(result.recipe?"success":"partial"):"error");setVideoAnalysisResult({videoId:video.id,status,source:(analysis?.source??video.source) as Source,caption:analysis?.caption??"",transcript:analysis?.transcript??"",onScreenText:analysis?.onScreenText??"",thumbnail:analysis?.thumbnail??"",evidence:analysis?.evidence??result.recipe?.evidence??[],recipe:result.recipe??null,message:analysis?.message??result.message??(result.recipe?"Analysis completed. Review the proposed recipe before saving.":"Analysis completed without a recipe."),error:analysis?.error??result.error??(!response.ok?`Analysis failed with HTTP ${response.status}`:""),retrievedAt:analysis?.retrievedAt??new Date().toISOString(),diagnostics:analysis?.diagnostics??null});setVideoAnalysisStage(status==="success"?"Analysis complete":status==="partial"?"Analysis completed with limited results":"Analysis failed")}catch(error){setVideoAnalysisResult({videoId:video.id,status:"error",source:video.source,caption:"",transcript:"",onScreenText:"",thumbnail:"",evidence:[],recipe:null,message:"ReelRecall could not complete this analysis.",error:error instanceof Error?error.message:"Reel analysis failed",retrievedAt:new Date().toISOString(),diagnostics:null});setVideoAnalysisStage("Analysis failed")}finally{setVideoAnalysisBusy(false)}}
  ...
  async function runFoodRecipeAudit(){
    ...
    const queue=videos.filter(video=>video.category==="Food"&&video.status==="available"&&hasExternalVideoUrl(video.url));
    setRecipeAuditOpen(true);

    if(!queue.length){
      setRecipeAudit(current=>({...current,running:false,total:0,completed:0,items:[]}));
      setToast("No linked Food videos are available to audit");
      return;
    }
  ...
  {!ready?<div className="empty">Loading…</div>:filtered.length?<section className="grid">{filtered.map(v=><VideoCard key={v.id} video={v} playing={playingId===v.id} onPlay={()=>setPlayingId(v.id)} onEdit={()=>openEdit(v)} onDelete={()=>setVideos(a=>a.filter(x=>x.id!==v.id))} onFavorite={()=>setVideos(a=>a.map(x=>x.id===v.id?{...x,favorite:!x.favorite}:x))} onStatus={()=>{setPlayingId(null);setVideos(a=>a.map(x=>x.id===v.id?{...x,status:x.status==="available"?"unavailable":"available"}:x));setToast(v.status==="available"?"Moved to Archive":"Restored to library")}} onRecipe={()=>openRecipe(v.id)} onManualRecipe={()=>openManualRecipe(v)} recipeLoading={recipeLoadingId===v.id} onAnalyzeVideo={()=>{if(videoAnalysisResult?.videoId===v.id&&!videoAnalysisBusy){setVideoAnalysisId(v.id);setVideoAnalysisOpen(true)}else void analyzeReelVideo(v)}} videoAnalyzing={videoAnalysisBusy&&videoAnalysisId===v.id} analysisReady={videoAnalysisResult?.videoId===v.id&&!videoAnalysisBusy}/>)}</section>:<section className="empty"><div>🎬</div><h2>{showArchive?"Archive is empty":videos.length?"No videos match":"Your ReelRecall library is ready"}</h2><p>{showArchive?"Videos marked unavailable will appear here.":"Add a Food/menu item manually or import supported video links."}</p></section>}
  <style>{`
...
.dialog.video-editor-dialog::before,
.dialog.video-editor-dialog::after{
  display:none!important;
  content:none!important;
}
.dialog.video-editor-dialog input,
.dialog.video-editor-dialog select,
.dialog.video-editor-dialog textarea{
  background:#ffffff!important;
  opacity:1!important;
}
`}</style>
  {dialogOpen?<div className="dialog-backdrop video-editor-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setDialogOpen(false)}}><div className="dialog video-editor-dialog"><button type="button" className="dialog-close" onClick={()=>setDialogOpen(false)}>×</button><p className="kicker">{editingId?"EDIT ITEM":"ADD ITEM"}</p><h2>{editingId?"Edit saved item":"Add to ReelRecall"}</h2><p>A video link is optional. For a Food item, you can save the dish now and add its recipe manually later.</p><form onSubmit={saveVideo} className="recipe-edit-form"><label>Title<input value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder="e.g. Kerala Style Moru Curry" required autoFocus/></label><label>Video link <small>Optional — Facebook, Instagram, YouTube, or another URL</small><input value={form.url} onChange={e=>setForm({...form,url:e.target.value})} placeholder="https://… or leave blank"/></label><div className="form-row"><label>Category<select value={form.category} onChange={e=>setForm({...form,category:e.target.value,subcategory:""})}>{categories.map(c=><option key={c.id} value={c.name}>{c.name}</option>)}</select></label><label>Subcategory<select value={form.subcategory} onChange={e=>setForm({...form,subcategory:e.target.value})} disabled={!formSubs.length}><option value="">No subcategory</option>{formSubs.map(sub=><option key={sub} value={sub}>{sub}</option>)}</select></label></div><label>Tags <small>Comma separated</small><input value={form.tags} onChange={e=>setForm({...form,tags:e.target.value})} placeholder="quick, vegetarian, dinner"/></label><label>Notes<textarea rows={4} value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Optional notes"/></label><label>Status<select value={form.status} onChange={e=>setForm({...form,status:e.target.value as Status})}><option value="available">Available</option><option value="unavailable">Unavailable</option></select></label><div className="analysis-actions"><button type="submit" className="primary" disabled={librarySaving}>{librarySaving?"Saving to database…":editingId?"Save Changes":"Add Item"}</button><button type="button" onClick={()=>setDialogOpen(false)} disabled={librarySaving}>Cancel</button></div></form></div></div>:null}
  {manualRecipeVideoId&&manualRecipeDraft?<div className="dialog-backdrop recipe-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)closeManualRecipe()}}><div className="dialog recipe-dialog"><button type="button" className="dialog-close" onClick={closeManualRecipe}>×</button><p className="kicker">MANUAL RECIPE</p><h2>{videos.find(v=>v.id===manualRecipeVideoId)?.title??"Recipe"}</h2>{hasExternalVideoUrl(videos.find(v=>v.id===manualRecipeVideoId)?.url??"")?<div className="recipe-footer recipe-editor-retry-bar"><button type="button" className="primary" onClick={()=>void retryDescriptionIntoManualEditor()} disabled={recipeLoadingId===manualRecipeVideoId}>{recipeLoadingId===manualRecipeVideoId?"Retrying description…":"↻ Retry from Description"}</button><small className="recipe-disclaimer">Read the saved video description and prefill the maximum reliable recipe details below. Nothing is saved until you click Save Recipe.</small></div>:<div className="recipe-footer recipe-editor-retry-bar"><button type="button" disabled title="This item has no external video link or description source">↻ Retry from Description — no video link</button></div>}<div className="recipe-edit-form"><label>Recipe title<input value={manualRecipeDraft.title} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,title:e.target.value})}/></label><div className="form-row"><label>Servings<input value={manualRecipeDraft.servings} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,servings:e.target.value})}/></label><label>Prep / cooking time<input value={manualRecipeDraft.prepTime} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,prepTime:e.target.value})}/></label></div><label>Ingredients <small>One ingredient per line · required for Grocery List</small><textarea rows={10} value={manualRecipeDraft.ingredients} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,ingredients:e.target.value})} placeholder={"1 cup yogurt\n2 eggs\n1 tsp salt"}/></label><label>Steps <small>One step per line · optional</small><textarea rows={8} value={manualRecipeDraft.steps} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,steps:e.target.value})}/></label><label>Notes<textarea rows={4} value={manualRecipeDraft.notes} onChange={e=>setManualRecipeDraft({...manualRecipeDraft,notes:e.target.value})}/></label><div className="analysis-actions"><button type="button" className="primary" onClick={saveManualRecipe}>Save Recipe</button><button type="button" onClick={closeManualRecipe}>Cancel</button></div></div></div></div>:null}
