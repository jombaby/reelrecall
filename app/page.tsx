...
type GroceryListResult = { menuItems:{schedule:string;videoId:string;title:string;ingredients:string[]}[]; summary:{item:string;quantity:string;usedBy:string[]}[]; generatedAt:string };
...
function WeeklyMenuPlanner({videos,onClose,onRecipeSaved,onOpenRecipe}:{videos:Video[];onClose:()=>void;onRecipeSaved:(videoId:string,recipe:Recipe)=>void;onOpenRecipe:(videoId:string)=>void}){
  ...
  async function saveMenu(){if(!items.length){setStatus("Generate a menu before saving.");return}const now=new Date().toISOString(),defaultName=`Week of ${weekTitle(weekStart)}`,name=menuName.trim()||defaultName;if(activeSavedId){const next=savedMenus.map(menu=>menu.id===activeSavedId?{...menu,name,weekStart,items,updatedAt:now}:menu);if(await persist(next,"Weekly menu updated."))setMenuName(name);return}const menu:SavedWeeklyMenu={id:crypto.randomUUID(),name,weekStart,items,createdAt:now,updatedAt:now};if(await persist([menu,...savedMenus],"Weekly menu saved.")){setActiveSavedId(menu.id);setMenuName(name);setRecipeCheckOpen(false)}}
  function loadMenu(id:string){const menu=savedMenus.find(item=>item.id===id);if(!menu)return;setActiveSavedId(menu.id);setMenuName(menu.name);setWeekStart(menu.weekStart);setItems(menu.items);resetDerived();setStatus(`Loaded ${menu.name}.`)}
  ...
  return <div className="dialog weekly-menu-dialog">
    ...
    <div className="weekly-menu-footer"><small>🗒 marks saved recipes; ✎ lets you add a missing recipe manually without leaving Weekly Menu. Click a thumbnail to open the reel.</small><button type="button" onClick={onClose}>Close</button></div>
    ...
    {groceryList?<div className="weekly-grocery-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setGroceryList(null)}}><div className="weekly-grocery-panel"><button className="dialog-close" onClick={()=>setGroceryList(null)}>×</button><p className="kicker">WEEKLY GROCERY LIST</p><h2>{menuName||`Week of ${weekTitle(weekStart)}`}</h2><p className="weekly-grocery-intro">Ingredients are shown by scheduled menu item first, followed by a consolidated shopping summary.</p><div className="weekly-grocery-items">{groceryList.menuItems.map((item,index)=><section key={`${item.videoId}-${index}`}><div><strong>{item.schedule}</strong><span>{item.title}</span></div><ul>{item.ingredients.map((ingredient,i)=><li key={`${i}-${ingredient}`}>{ingredient}</li>)}</ul></section>)}</div><section className="weekly-grocery-summary"><h3>Shopping summary</h3><ul>{groceryList.summary.map((item,index)=><li key={`${index}-${item.item}`}><div><strong>{item.item}</strong>{item.quantity?<span>{item.quantity}</span>:null}</div>{item.usedBy.length?<small>Used by: {item.usedBy.join(", ")}</small>:null}</li>)}</ul></section><div className="analysis-actions"><button type="button" className="primary" onClick={()=>void copyGroceryList()}>Copy Grocery List</button><button type="button" onClick={()=>void generateGroceryList()} disabled={groceryLoading}>{groceryLoading?"Refreshing…":"Regenerate"}</button><button type="button" onClick={()=>setGroceryList(null)}>Close</button></div></div></div>:null}
  </div>
}
...
export default function Home(){
  ...
  function saveRecipeEdits(video:Video){if(!video.recipe||!recipeDraft)return;const ingredients=recipeDraft.ingredients.split("\n").map(item=>item.trim()).filter(Boolean),steps=recipeDraft.steps.split("\n").map(item=>item.trim()).filter(Boolean);if(!ingredients.length||!steps.length){setToast("Recipe needs at least one ingredient and one step");return}const updated:Recipe={...video.recipe,title:recipeDraft.title.trim()||video.title,servings:recipeDraft.servings.trim(),prepTime:recipeDraft.prepTime.trim(),ingredients,steps,notes:recipeDraft.notes.trim()};setVideos(current=>current.map(item=>item.id===video.id?{...item,recipe:updated}:item));setRecipeEditing(false);setRecipeDraft(null);setToast("Recipe changes saved")}
