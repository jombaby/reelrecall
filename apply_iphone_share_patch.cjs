...
const routePath = path.join(root, "app", "api", "share", "route.ts");

if (!fs.existsSync(proxyPath)) {
  console.error("Error: proxy.ts was not found. Run this script from the ReelRecall project folder.");
  process.exit(1);
}

let proxy = fs.readFileSync(proxyPath, "utf8");
if (!proxy.includes('/api/share')) {
  const formatted = 'path.startsWith("/api/auth") ||';
  const compact = 'path.startsWith("/api/auth")||';
  if (proxy.includes(formatted)) {
    proxy = proxy.replace(formatted, 'path.startsWith("/api/auth") || path.startsWith("/api/share") ||');
  } else if (proxy.includes(compact)) {
    proxy = proxy.replace(compact, 'path.startsWith("/api/auth")||path.startsWith("/api/share")||');
  } else {
    console.error("Error: Could not locate the public-route list in proxy.ts. No files were changed.");
    process.exit(1);
  }
...
type SavedVideo={id:string;title:string;url:string;category:string;subcategory:string;tags:string[];notes:string;favorite:boolean;source:string;addedAt:string;status:string;categoryLocked:boolean;tagsLocked:boolean;aiStatus:string};
...
console.log("ReelRecall iPhone sharing patch applied successfully.");
console.log("Updated: proxy.ts");
console.log("Created: app/api/share/route.ts");
